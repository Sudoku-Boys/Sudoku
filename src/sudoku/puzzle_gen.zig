...
pub fn generate_puzzle_safe(comptime K: u16, comptime N: u16, comptime solver: solve.Solvers, gen_clues: usize, clues: usize, allocator: std.mem.Allocator, comptime attempts: usize) board.Board(K, N, .HEAP) {
    var i: usize = attempts;

    while (i > 0) {
        return generate_puzzle(K, N, solver, gen_clues, clues, allocator) catch {
            i -= 1;
            continue;
        };
    }
...
pub fn generate_puzzle(comptime K: u16, comptime N: u16, comptime solver: solve.Solvers, gen_clues: usize, clues: usize, allocator: std.mem.Allocator) !board.Board(K, N, .HEAP) {
